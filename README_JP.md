...
クローン翻訳吹き替えツールは、プリセット話者による SI ナレーションではなく、翻訳後の吹き替え制作向けです：

- 元動画を文字起こしし、話者ごとにセリフを分離します
- 検出した各話者から参照音声サンプルを抽出します
- 認識したセリフをターゲット言語へ翻訳します
- OmniVoice を使い、対応する話者のクローン音声で翻訳後のセリフを合成します
- クローン音声をタイムラインに合わせて `<動画名>.si.wav` として組み立てます
- 再ミックスは2つのモードに対応します：
  - 同時通訳モード：元音声を残し、クローン/翻訳音声を重ねて `_SI.mp4` を出力します
  - 吹き替えモード：Bandit-v2 で元の声を除去し、音楽・効果音の背景を残してクローン音声を追加し、`_DUB.mp4` を出力します
- 単一ファイルと一括フォルダ、既存出力のスキップ、中間ファイル保持、ローカル ECAPA クラスタリング、任意の pyannote 話者分離、平坦なナレーション・文単位で合わせる・抑揚に追従などの出力音量モードに対応します

音声クローンの品質は、元音声の品質、話者分離の精度、参照サンプルの選択、短い翻訳文に対するモデル挙動に左右されます。最終出力として使う前に、生成された `.si.wav`、`_SI.mp4`、`_DUB.mp4` を確認してください。
...
- `クローン翻訳吹き替え`：動画のセリフを文字起こし・翻訳し、話者ごとのクローン音声で `<動画名>.si.wav` を生成して `_SI.mp4` または `_DUB.mp4` に再ミックス
...
- クローン翻訳吹き替えは `<動画名>.si.wav` を生成し、再ミックス後は同時通訳モードで `_SI.mp4`、吹き替えモードで `_DUB.mp4` を出力します
