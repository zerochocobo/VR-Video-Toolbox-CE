...
- 基本 Python パッケージ：`Pillow`、`pyinstaller`、`ffmpy3`、`faster-whisper`、`numpy>=1.26,<2.1`、`auditok`、`huggingface-hub`、`keyring`、`requests`、`av`、`fastapi`、`uvicorn`
...
- 内蔵 AI/GPU パッケージ：PyTorch `cu128` wheel インデックスの `torch==2.8.0` と `torchvision==0.23.0`、および `ultralytics==8.4.4`、`mmengine==0.10.7`
...
`pip` で手動インストールする場合は、CUDA 関連パッケージのバージョンを `pyproject.toml` と一致させ、PyTorch / torchvision は `https://download.pytorch.org/whl/cu128` からインストールしてください。
